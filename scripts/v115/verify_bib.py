...
def doi_candidates(doi: str) -> List[str]:
    """The DOI as given, then progressively trimmed of reference-manager export junk.

    Reference managers append markers to a DOI on export, e.g.
    "10.1091/MBC.E11-02-0136;SUBPAGE:STRING:FULL", which makes both the PubMed and the
    Crossref DOI lookups miss and sends the entry to MANUAL_CHECK even though the DOI
    itself is correct. Semicolons are legal in DOIs (many Wiley DOIs end ";2-J"), so a
    truncated form is only ever tried as a fallback, after the literal one.
    """
    out = [doi]
    trimmed = re.sub(r';\s*[A-Z]+:.*$', '', doi).strip().rstrip('.,;')
    if trimmed and trimmed not in out:
        out.append(trimmed)
    bare = doi.split(';')[0].strip().rstrip('.,')
...
class BibTexEnhancer:
    ...
    def convert_crossref_to_meta(self, cr_item: Dict) -> Dict[str, str]:
        ...
        meta['title'] = (cr_item.get('title') or [''])[0]
        meta['journal'] = (cr_item.get('container-title') or [''])[0]
        ...
        if pub and pub.get('date-parts') and pub['date-parts'][0]:
    ...
    def search_pubmed(self, entry: Dict[str, str]) -> Tuple[Optional[Dict], str]:
        ...
        if 'doi' in entry and entry['doi']:
            ...
            for candidate in doi_candidates(clean_doi):
                res = self.fetch_by_doi(candidate)
                if res:
                    fetched_title = res.get('Title', '')
                    fetched_first = res.get('AuthorList', [None])[0] if res.get('AuthorList') else ""
                    if verify_match_precision(draft_title, fetched_title, entry.get('author', ''), fetched_first):
                ...
                cr_res = self.search_crossref_by_doi(candidate)
                if cr_res:
        ...
        stopwords = {'and', 'in', 'of', 'the', 'for', 'with', 'on', 'at', 'to', 'from', 'by', 'a', 'an', 'is', 'are', 'its', 'through', 'during', 'between', 'into', 'over', 'under', 'than', 'this', 'that', 'these', 'those', 'which', 'who', 'whom', 'been', 'being', 'have', 'has', 'had', 'was', 'were', 'will', 'would', 'can', 'could', 'may', 'might', 'should', 'about', 'above', 'after', 'before', 'but', 'or', 'not', 'as', 'be'}
    ...
    def process(self):
        ...
        print(f"Verified via PubMed:   {self.stats['method_pmid'] + self.stats['method_doi'] + self.stats['method_pubmed_title'] + self.stats['method_pmc']}")
