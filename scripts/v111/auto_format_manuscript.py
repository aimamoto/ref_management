...
def main():
    ...
    # Required scripts
    scripts = ["scan_raw_refs.py", "verify_bib.py", "apply_citations.py"]
    ...
    step1 = subprocess.run([python_exe, "scan_raw_refs.py", str(doc_path)])
    if step1.returncode != 0:
        print("\n❌ ERROR: Pipeline failed during Step 1 (Scanning).")
        sys.exit(1)

    # Determine the extracted output filename based on scan_raw_refs.py logic
    ...
    step2 = subprocess.run([python_exe, "verify_bib.py", str(extracted_bib)])
