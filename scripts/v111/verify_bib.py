...
def get_author_surname(author_str: str) -> str:
    """Intelligently extracts the author's surname regardless of style formatting."""
    if not author_str:
        return ""
    ...
    if ',' in author_str:
        ...
        words = [w.strip() for w in author_str.split() if w.strip()]
        if not words:
            return ""
        if len(words) == 1:
            return words[0]
        ...
        if len(last_word_clean) <= 2 or last_word_clean.lower() in ['jr', 'sr', 'iii', 'ii']:
            return words[0]
        else:
            return last_word
...
def verify_match_precision(draft_title: str, fetched_title: str, draft_author: str, fetched_first_author: str) -> bool:
    """
    Verifies if the fetched metadata matches the draft citation with high precision
    by checking both title similarity and first author surname similarity.
    """
    # 1. Title check
    ...
    # 2. Author check (if both are available)
...
class BibTexEnhancer:
    ...
    def search_crossref(self, title: str) -> Optional[List[Dict]]:
    ...
    def search_pubmed(self, entry: Dict[str, str]) -> Tuple[Optional[Dict], str]:
        ...
        # Keep words separated by replacing hyphens and slashes with spaces
        clean_title_str = title.replace('-', ' ').replace('–', ' ').replace('/', ' ')
        search_title = re.sub(r'[^\w\s]', '', clean_title_str)
        ...
        if len(search_title) > 10:
            ...
            cr_items = self.search_crossref(search_title)
            if cr_items:
    ...
    def process(self):
        ...
        # =====================================================================
        # --- NEW: Post-Processor for Modern Online/MDPI Journals and Authors ---
        print("\nEnrichment complete. Finalizing formatting...")
        for entry in bib_database.entries:
            # Clean author field if it exists and contains commas but no 'and'
