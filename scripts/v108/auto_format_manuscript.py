...
def main():
    ...
    # Required scripts
    scripts = ["scan_raw_refs_r3.py", "verify_bib_r3.py", "apply_citations_r3.py"]
    ...
    step1 = subprocess.run([python_exe, "scan_raw_refs_r3.py", str(doc_path)])
    if step1.returncode != 0:
        print("\n❌ ERROR: Pipeline failed during Step 1 (Scanning).")
        sys.exit(1)

    # Determine the extracted output filename based on scan_raw_refs_r3.py logic
    ...
    step2 = subprocess.run([python_exe, "verify_bib_r3.py", str(extracted_bib)])
    ...
    step3 = subprocess.run([
        python_exe, "apply_citations_r3.py", 
        str(verified_bib), 
        str(doc_path), 
        "--csl", str(csl_path)
    ])
