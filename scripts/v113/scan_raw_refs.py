...
def extract_citation_parts(text: str) -> dict:
    ...
    if year_match:
        ...
        if is_near_front:
            ...
            # Strip leftover closing punctuation from a parenthesised year, e.g. '(2003).'
            # otherwise the sentence splitter takes ')' as the title -> empty title -> NOT_FOUND
    ...
    for item in items_to_process:
        ...
        if not meta:
            if item['status'] == "FAIL_DOI_LOOKUP": 
                notes.append("DOI not in PubMed/Crossref")
                status = "MANUAL_CHECK"
            else: 
                year_match = find_publication_year(text)
