...
def extract_citation_parts(text):
    ...
    if year_iter:
        ...
        # SMART SLICER: Differentiate Vancouver vs Author-Year formats
        if selected_year_match.start() > len(text) / 2:
            # Year is at the end (Vancouver style). Title is BEFORE the year.
            snippet = text[:selected_year_match.start()].strip()
        else:
            # Year is near the beginning. Title is AFTER the year.
            snippet = text[selected_year_match.end():].strip()
            
        data["title_snippet"] = re.sub(r'^[\s\)\.,:;-]+|[\s\.,:;-]+$', '', snippet)
    else:
        data["title_snippet"] = text.strip()

    skip_words = {'et', 'al', 'in', 'the', 'pmid', 'doi', 'vol', 'no', 'and', '&', 'eds', 'editor', 'page', 'pp', 'references', 'bibliography'}
    for w in text.split():
        clean = clean_word(w)
...
def search_pubmed_by_metadata(parts):
    ...
    if not candidates and parts["author"] and parts["year"]:
        try:
            handle = Entrez.esearch(db="pubmed", term=f"{parts['author']}[1au] AND {parts['year']}[pdat]", retmax=5)
            r = Entrez.read(handle)
            handle.close()
            if r['IdList']: candidates.extend(r['IdList'])
        except Exception: pass
...
def analyze_document(file_path):
    ...
    for item in items_to_process:
        ...
        if not meta:
            ...
            # Full-text validation to prevent false MISMATCH warnings
...
def save_outputs(csv_data, txt_lines, bib_entries, base_name):
    ...
    print(f"  [BibTeX] -> {bib_name}  <-- USE THIS FOR verify_bib_r2.py")
